...
public class LTDescr
{
	...
	public override string ToString(){
		return (trans!=null ? "name:"+trans.gameObject.name : "gameObject:null")+" toggle:"+toggle+" passed:"+passed+" time:"+time+" delay:"+delay+" _direction:"+direction+" from:"+from+" to:"+to+" diff:"+diff+" type:"+type+" ease:"+easeType+" useEstimatedTime:"+useEstimatedTime+" id:"+id+" hasInitiliazed:"+hasInitiliazed;
	}
...
//		Debug.Log ("tween:" + this+ " dt:"+dt);
	...
	* 
	* @method setAxis
	* @param {Vector3} axis either the tween rotates around, or the _direction it faces in the case of setOrientToPath
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
...
//		val = (val * val * val * val - 2f);
	...
	*/
	...
	/**
	* Set the _direction of a tween -1f for backwards 1f for forwards (currently only bezier and spline paths are supported)
	* @method setDirection
	* @param {float} _direction:float the _direction that the tween should run, -1f for backwards 1f for forwards
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	...
	*/
	...
	public LTDescr setDirection( float direction ){
		if(this.direction!=-1f && this.direction!=1f){
			Debug.LogWarning("You have passed an incorrect _direction of '"+direction+"', _direction must be -1f or 1f");
			return this;
		}
